...
def startup():
    ...
    average_survival_rate = (np.mean([item[0]
                                      for item in titanic_array]) * 100)
...
def submit_new_profile():
    ...
    if request.method == 'POST':
        ...
        user_designed_passenger = [[age, sibsp, parch, fare, isfemale, pclass_Second, pclass_Third, pclass_nan, cabin_B, cabin_C, cabin_D, cabin_E, cabin_F, cabin_G, cabin_T,
                                    cabin_Unknown, cabin_nan, embarked_Q, embarked_S, embarked_Unknown, embarked_nan, title_Master, title_Miss, title_Mr, title_Mrs, title_Rev, title_Unknown, title_nan]]
        ...
        performance = [average_survival_rate,
                       probability_of_surviving_fictional_character]
        ...
        plt.title('His/her chance of surviving is...? \n ' +
                  str(round(probability_of_surviving_fictional_character, 2)) + '% ')
        img = io.BytesIO()
